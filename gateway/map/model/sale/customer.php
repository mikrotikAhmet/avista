...
class ModelSaleCustomer extends Model {
	public function addCustomer($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', mobile = '" . $this->db->escape($data['mobile']) . "',fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW()");
	...
	public function editCustomer($customer_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', certificate_id = '".(int) $data['certificate_id']."',firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', mobile = '" . $this->db->escape($data['mobile']) . "',telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function deleteCustomer($customer_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "certificate WHERE certificate_id = '" . (int)$customer_data['certificate_id'] . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "customer_identity WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function approve($customer_id) {
		...
		if ($customer_info) {
			...
			if ($application_info) {
				...
				$application_url = $application_info['url'] . 'index.php?route=account/login';
			} else {
				$application_name = $this->config->get('config_name');
				$application_url = HTTP_PORTAL . 'index.php?route=account/login';
			}

			$message  = sprintf($this->language->get('text_approve_welcome'), $application_name) . "\n\n";
			...
			$mail->setSubject(sprintf($this->language->get('text_approve_subject'), $application_name));
	...
	public function getTotalCustomersApprovalRequest() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE account_status = '1' AND approved = '0'");
    ...
    public function updateDocumentStatus($document_id, $status_id){
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "customer_document SET status = '".(int) $status_id."' WHERE document_id = '" . (int)$document_id . "'");

    }

    public function getTotalDocuments($customer_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer_document WHERE customer_id = '" . (int)$customer_id . "'");

        return $query->row['total'];
    }

    public function getTotalDocumentsByCustomerId($customer_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer_document WHERE customer_id = '" . (int)$customer_id . "'");

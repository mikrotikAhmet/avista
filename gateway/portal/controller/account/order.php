	...
	public function index(){
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'contract'=>($result['contract_no'] > 0 ? '<span class="text-success">Issued</span>' : '<span class="text-danger">Not Issued</span>'),
...
$data['order'] = $this->model_sale_order->getOrder($order_id);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/order_detail.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/order_detail.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/account/order_detail.tpl', $data));
		}

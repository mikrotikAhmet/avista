...
class ModelSaleOrder extends Model {
    ...
    public function addOrder($data,$order_id)
    {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET order_id = '".(int) $order_id."',invoice_prefix = '" . $this->db->escape($data['invoice_prefix']) . "', application_id = '" . (int)$data['application_id'] . "', application_name = '" . $this->db->escape($data['application_name']) . "', application_url = '" . $this->db->escape($data['application_url']) . "', customer_id = '" . (int)$data['customer_id'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . (isset($data['fax']) ? $this->db->escape($data['fax']) : '') . "',  comment = '" . (isset($data['comment']) ? $this->db->escape($data['comment']) : '')  . "', total = '" . (isset($data['total']) ? (float)$data['total'] : 0) . "', request = '" . (float)$data['request'] . "',commission = '" . (isset($data['commission']) ? (float)$data['commission'] : 0) . "',  tracking = '" . (isset($data['tracking']) ? $this->db->escape($data['tracking']) : '')  . "', language_id = '" . (int)$data['language_id'] . "', currency_id = '" . (int)$data['currency_id'] . "', currency_code = '" . $this->db->escape($data['currency_code']) . "',settlement = '".(int) $data['settlement']."', bank_id = '".(int) $data['bank_id']."', currency_value = '" . (float)$data['currency_value'] . "', ip = '" . $this->db->escape($data['ip']) . "', forwarded_ip = '" . $this->db->escape($data['forwarded_ip']) . "', user_agent = '" . $this->db->escape($data['user_agent']) . "', accept_language = '" . $this->db->escape($data['accept_language']) . "', issuer_name = '".$this->db->escape($data['issuer_name'])."', date_added = NOW(), date_modified = NOW()");

	    // Products
	    $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$data['product_id'] . "', name = '" . (isset($data['other']) ? $this->db->escape($data['other']) : $this->db->escape($data['product_name'])) . "', price = '" . (isset($data['price']) ? (float)$data['price'] : 0) . "'");
